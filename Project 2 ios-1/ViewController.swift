...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    private var fromToggle = false
    private var storedWeatherResponse: WeatherResponse?
    private var isSearching = false
    private var searchedWeather: [WeatherResponse] = []
   ...
   @IBAction func onSwitchChange(_ sender: UISwitch) {
    ...
    @IBAction func btnCities(_ sender: UIButton) {
        print("Cities")
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "SearchedWeatherViewController") as! SearchedWeatherViewController
        vc.setupData(items: searchedWeather, isCelsius: isCelsius)
...
//        let code = weatherResponse.current.condition.code
        print("ConditionName:- \(weatherResponse)")
        if !fromToggle {
            searchedWeather.append(weatherResponse)
        }
        DispatchQueue.main.async {
            ...
            self.lblWeatherCondition.text = weatherResponse.current.condition.text
        }
        fromToggle = false
